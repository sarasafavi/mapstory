...
def pre_save_profile(instance, sender, **kwargs):
    if Profile.objects.filter(id=instance.id).exists():
        cur_pwd = Profile.objects.get(id=instance.id).password
...
def set_mapstory_notifications():
    ...
    signals.post_save.connect(favorite_post_save, sender=Favorite)
